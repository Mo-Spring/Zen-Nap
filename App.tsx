...
export default function App() {
  ...
  const stopAllAudio = () => {
    ...
    if (Capacitor.isNativePlatform()) {
      AlarmChannel.stopAlarmAudio().catch(e => console.warn("stopAlarmAudio failed", e));
    }
    setPlayingAudioPath(null);
  };
...
const playAlarmMusic = async (trackPath: string, loop: boolean = true) => {
          ...
          try {
              ...
              if (activeUploadContext.field === 'wakeUp') {
                  ...
                  if (activeUploadContext.modeId === 'ALL') {
                      ...
                      MODES.forEach(mode => { newGuides[mode.id] = track; });
  ...
  const startTimerInternal = async (durationMinutes: number) => {
    ...
    if (!sessionStats) {
        setSessionStats({ startTime: now, endTime: end, durationSeconds: durationSec });
    } else {
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(diff) > SWIPE_THRESHOLD) {
      if (diff > 0) { handleModeSelect(selectedModeIndex + 1); }
      else { handleModeSelect(selectedModeIndex - 1); }
    }
  ...
  return (
    <div className="relative w-full h-full font-light touch-none">
            ...
            <div className="pb-24 w-full flex flex-col items-center justify-center">
                <div className="relative flex items-center justify-center">
                    <svg className="absolute w-[88px] h-[88px] pointer-events-none transform -rotate-90">
                        <circle r="42" cx="44" cy="44" fill="transparent" stroke="rgba(255,255,255,0.1)" strokeWidth="2" />
                        <circle r="42" cx="44" cy="44" fill="transparent" stroke="white" strokeWidth="3"
      ...
      {appState === AppState.ALARM && (
        <div className="absolute inset-0 z-30 flex flex-col items-center justify-between pb-16 pt-20 overflow-hidden">
